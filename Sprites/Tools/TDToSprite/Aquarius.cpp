...
static const char*	gsc_szCharExtension		= ".chr";	// Char extension;
...
bool Aquarius::initialize(const std::string& _strPrefix)
{
	...
	if (false == loadSprites(_strPrefix))
	{
		return	false;
	}
...
bool Aquarius::loadSprites(const std::string& _strPrefix)
{
	File::Ptr	pFile	= File::create();
	...
	SHAREDBUFFER(m_pScreenBuffer, uint8_t, m_iChars);
	...
	SHAREDBUFFER(m_pColorBuffer, uint8_t, m_iChars);
	...
	for (int iLoop = 0; iLoop < m_iChars; ++iLoop)
	{
		pScreen[iLoop]	= *pMap;
		pMap++;
...
bool Aquarius::save(const std::string& _strPrefix)
{
	...
	strFilename	= _strPrefix;
	strFilename	+= gsc_szCharExtension;
	...
	// Write screen
	if (false == pFile->writeBuffer(m_pScreenBuffer.get(), m_iChars))
	{
	...
	// Write color
	if (false == pFile->writeBuffer(m_pColorBuffer.get(), m_iChars))
	{
